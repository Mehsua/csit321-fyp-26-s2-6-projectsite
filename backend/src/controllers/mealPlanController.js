...
async function generate(req, res, next) {
  try {
    ...
    const plan = await service.generateAndSavePlan(req.user.id, sessionIngredients, parsedDays);
...
async function deletePlanItem(req, res, next) {
  try {
    const service = new MealPlanService();
    await service.deletePlanItem(req.user.id, req.params.itemId);
    res.status(204).send();
  } catch (err) {
...
async function addItem(req, res, next) {
  try {
    ...
    const service = new MealPlanService();
    const planId = await service.addItemToPlan(req.user.id, recipeId, dayNumber);
